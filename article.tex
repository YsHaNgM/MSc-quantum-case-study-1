...
Now, imagine that I asked you to compute the energy surface of a given molecule; how would you carry out this task? You would execute an algorithm that produces a numerical value. In Scalar Quantum of Molecular Energies two techniques used to determine the energy surface for a hydrogen molecule are discussed which are: 1) The Variational Quantum Eigensolver (VQE) and 2) The Quantum Phase Estimation Algorithm (PEA) which constitutes of the ‘original algorithm for quantum chemistry’.
...
Similarly to the VQE algorithm, one must ensure that the system is prepared in an initial state, but with the exception that the state must have a sufficient overlap with the ground state.

Similarly, to the VQE algorithm, one must ensure that the system is prepared in an initial state, but with the exception that the state must have a sufficient overlap with the ground state. Once the state is prepared in our desired state, an approximation is applied to the operator. Consequently, the initial state is mapped onto a superposition state, a state that is made up of different vectors. After that, the state is acted on in turn inducing a phase on the system. Since the way in which the system evolves is controlled, it is now entangled with the initialised state. Next, we measure the phase obtained corresponding to each value of energy. The system as a result collapses to a single state with an associated probability. We repeat this algorithm for a higher number of ‘special’ qubits until the lowest eigenenergy is obtained, or more precisely the energy of the ground state.
...
\section{Technical Boxes}
\section{VQE}
\section{PEA}
\section{Experimental realisation (SQUID)}
